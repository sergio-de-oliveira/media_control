...
InputMessage(wParam, lParam, msg, hwnd)
{
	...
	if (Type = RIM_TYPEMOUSE)
	{
		...
		usButtonFlags:=NumGet(Buffer, 12+A_PtrSize*2, "UShort")	; Transition state of the mouse buttons.
		usButtonData:=NumGet(Buffer, offsets.usButtonData, "Short")		; Contains mouse wheel delta, if usButtonFlags=RI_MOUSE_WHEEL=0x0400
		AppendOutput("`r`nHND " . Handle . " MOUSE buttons " . usButtonFlags . " MOUSE wheel delta " . usButtonData . "`r`n")
		AppendOutput("`r`nHND " . Handle . " MOUSE LastX " . LastX . " LastY " . LastY . "`r`n")
	}
	else if (Type = RIM_TYPEKEYBOARD)
	{
		ScanCode := NumGet(Buffer, (8+A_PtrSize*2), "UShort") ;460390701
