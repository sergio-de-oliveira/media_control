class VMR{
    ...
    class VM_BUS_STRIP {
        ...
        setParameter(parameter, value){
            local func
            if parameter = "device" || "FadeTo" || "FadeBy" || "AppGain" || "AppMute" ;if parameter contains device,FadeTo,Label,FadeBy,AppGain,AppMute
        ...
        __setDevice(name,driver){
            if (!this.__isPhysical())
                return -4
            if driver != "wdm" || "mme" || "ks" || "asio" ;if driver not in wdm,mme,ks,asio
                return -5
        ...
        class stream{
            ...
            __Set(p_name,p_value){
                if(VMR.vban.stream.initiated) {
                    if p_name = "name"|| "ip" ;if p_name contains name, ip
    ...
    class recorder_base {
        ...
        __Set(p_name,p_value){
            local type:= "Float"
            if p_name = "goto"|| "load" ;if p_name contains goto,load
                type:= "String"
